function [Vchr, vchr, uchr, tchr, cpuchr, statuschr, optchr] = solve_orpd_chr2(casedata,model)
...
if (isempty(isShunt) && isempty(isTap))
    ...
    if (~isempty(isShunt) && isempty(isTap))
        [Vchr, vchr, uchr, cpuchr, statuschr, optchr] = solve_orpd_shunt_chr(casedata,model);
        tchr = [];
    end
    if (isempty(isShunt) && ~ isempty(isTap))
        [Vchr, vchr, tchr, cpuchr, statuschr, optchr] = solve_orpd_tap_chr2(casedata,model);
        uchr = [];
    end
    if (~isempty(isShunt) && ~isempty(isTap))
