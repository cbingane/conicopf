function [Vtcr, vtcr, utcr, ttcr, cputcr, statustcr, opttcr] = solve_orpd_tcr2(casedata,model)
...
if (isempty(isShunt) && isempty(isTap))
    ...
    if (isempty(isShunt) && ~isempty(isTap))
        [Vtcr, vtcr, ttcr, cputcr, statustcr, opttcr] = solve_orpd_tap_tcr2(casedata,model);
        utcr = [];
    end
    if (~isempty(isShunt) && ~isempty(isTap))
