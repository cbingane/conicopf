function [Vsdr, vsdr, usdr, tsdr, cpusdr, statussdr, optsdr] = solve_orpd_sdr2(casedata,model)
...
if (isempty(isShunt) && isempty(isTap))
    ...
    if (isempty(isShunt) && ~ isempty(isTap))
        [Vsdr, vsdr, tsdr, cpusdr, statussdr, optsdr] = solve_orpd_tap_sdr2(casedata,model);
        usdr = [];
    end
    if (~isempty(isShunt) && ~ isempty(isTap))
