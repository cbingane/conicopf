function [Vtcr, vtcr, utcr, ttcr, cputcr, statustcr, opttcr] = solve_orpd_tcr1(casedata,model)
...
if (isempty(isShunt) && isempty(isTap))
    ...
    if (~isempty(isShunt) && isempty(isTap))
        [Vtcr, vtcr, utcr, cputcr, statustcr, opttcr] = solve_orpd_shunt_tcr(casedata,model);
        ttcr = [];
    end
    if (isempty(isShunt) && ~isempty(isTap))
        [Vtcr, vtcr, ttcr, cputcr, statustcr, opttcr] = solve_orpd_tap_tcr1(casedata,model);
        utcr = [];
    end
    if (~isempty(isShunt) && ~ isempty(isTap))
