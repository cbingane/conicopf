function [optval, optsol, Vopt, uopt, topt, cpu, status] = solve_orpd_chr1(casedata,model)
...
if (isempty(isShunt) && isempty(isTap))
    ...
    if (~isempty(isShunt) && isempty(isTap))
        [optval, optsol, Vopt, uopt, cpu, status] = solve_orpd_shunt_chr(casedata,model);
        topt = [];
    end
    if (isempty(isShunt) && ~ isempty(isTap))
        [optval, optsol, Vopt, topt, cpu, status] = solve_orpd_tap_chr1(casedata,model);
        uopt = [];
    end
    if (~isempty(isShunt) && ~isempty(isTap))
