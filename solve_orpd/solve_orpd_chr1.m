function [Vchr, vchr, uchr, tchr, cpuchr, statuschr, optchr] = solve_orpd_chr1(casedata,model)
...
if (isempty(isShunt) && isempty(isTap))
    ...
    if (isempty(isShunt) && ~ isempty(isTap))
        [Vchr, vchr, tchr, cpuchr, statuschr, optchr] = solve_orpd_tap_chr1(casedata,model);
        uchr = [];
    end
    if (~isempty(isShunt) && ~isempty(isTap))
